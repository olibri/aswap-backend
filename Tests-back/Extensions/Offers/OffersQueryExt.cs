...
public static class OffersQueryExt
{
    ...
    string? fiat = null, string? mint = null, decimal? priceFrom = null, string[]? paymentMethod = null)
  {
    return q with
    {
      ...
      PriceFrom = priceFrom ?? q.PriceFrom,
  ...
  public static async Task<int> SeedOnlineSessionsAsync(this TestFixture f, int online = 1, int stale = 0,
    CancellationToken ct = default)
  {
    ...
    for (var i = 0; i < online; i++)
      list.Add(new SessionEntity
        { Wallet = $"w{i}", Ip = "1.1.1.1", StartedAt = now.AddMinutes(-5), LastSeenAt = now });
    for (var j = 0; j < stale; j++)
      list.Add(new SessionEntity
        { Wallet = $"s{j}", Ip = "2.2.2.2", StartedAt = now.AddMinutes(-120), LastSeenAt = now.AddMinutes(-15) });
  ...
  public static async Task<DashboardMetricsDto> ExecuteDashboardAsync(this TestFixture f, DateTime? from = null,
    DateTime? to = null, CancellationToken ct = default)
  {
    var controller = f.GetService<AdminController>();
    var query = new DashboardQuery(from, to);
  ...
  {
    return new DateTimeOffset(d.Date, TimeSpan.Zero).ToUnixTimeSeconds();
  }
